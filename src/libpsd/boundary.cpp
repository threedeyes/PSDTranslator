	...
	psd_double line_width, psd_double flatness)
{
	...
	psd_int closed;
	psd_int last, _this, next, second;
	psd_double dx, dy;
	...
	for(begin_idx = 0; vpath[begin_idx].code != PSD_ART_END; begin_idx = end_idx)
	{
		...
		*/
		...
		for(i = _this + 1; vpath[i].code == PSD_ART_LINETO; i++)
		{
			dx = vpath[i].x - vpath[_this].x;
			dy = vpath[i].y - vpath[_this].y;
		...
		while(vpath[next].code == PSD_ART_LINETO)
		{
			...
			if(vpath[next].code != PSD_ART_LINETO)
			{
				...
				if(closed &&
					vpath[_this].x == vpath[begin_idx].x &&
					vpath[_this].y == vpath[begin_idx].y)
				{
					psd_int j;
			...
			seg->points[curs].y < y + 1)
		{
			...
			if(y_top != y_bot)
			{
				...
				else
				{
					...
					for(; x < ix_max; x++)
					{
						_this = (psd_int)((seg->dir ? 16711680.0 : -16711680.0) * rslope *
							(x + 0.5 - x_min));
						xdelta = _this - last;
						last = _this;
